...
| riskKD output (token-wt α=1.0, 1 epoch) | same + `kl_inv` token weighting α=1.0 — fixed cross-size instability | ⬜ not pushed | `riskKD_output_tokenwt/` |
| **`llama-3.2-1b-riskkd-tokenwt-epoch1`** | **3-epoch token-wt riskKD, epoch-1 ckpt (step 902)** — best on the lm-eval suite (43.02 avg), so the one we kept | ✅ private | `riskKD_output_tokenwt_3ep/checkpoint-902/` |
| riskKD 3-epoch token-wt, ep2/ep3 ckpts | epochs 2 & 3 of the same run | ⬜ not pushed (ep2 not even evaluated) | `riskKD_output_tokenwt_3ep/checkpoint-{1804,2706}/` |

## What's been evaluated (lm-eval-harness, vLLM TP=8, gpu_memory_utilization=0.8, max_model_len=4096)

**Full table in `results/eval/SUMMARY.md`** (per-task JSONs alongside). Headline averages:

| Model | Avg over MMLU / TruthfulQA-MC2 / Winogrande / HellaSwag / GSM8K / ARC-C |
|---|---|
| SFT-ep1 (8B) | 62.12% |
| DPO-from-ep1 (8B) | 64.42% |
| riskKD 1ep, no token-wt (1B) | 42.45% |
| riskKD 1ep, token-wt α=1.0 (1B) | 43.15% |
| **riskKD 3ep token-wt, ep1 (1B) — kept, on HF** | **43.02%** |
| riskKD 3ep token-wt, ep3 (1B) | 42.99% |

**Read:** 8B SFT→DPO gave +2.3 avg (TruthfulQA +6.8, GSM8K +4.6). The 1B student is capacity-limited; the one signal that transferred is TruthfulQA ~48–49% (vs base 1B ~38%, ≈ +10). Token weighting buys +0.7 over un-weighted (and, more importantly, makes the cross-size run trainable at all). More epochs add nothing. Knowledge benchmarks unchanged (expected). ARC-C low for all (chat-format vs raw-loglikelihood mismatch — would close with `--apply_chat_template`).
...
## ✅ DONE — 3-epoch token-weighted riskKD + per-epoch eval

Ran the 3-epoch token-wt riskKD (`num_train_epochs=3`, save per epoch → checkpoint-902/1804/2706, `output_dir=riskKD_output_tokenwt_3ep`, else identical to the 1-ep token-wt recipe). train_runtime ≈ 1h38m, final `train_loss` 0.6762. Reward margins grew across epochs: ep1 ≈ +1.0–1.7, ep2 ≈ +2.0, ep3 ≈ +2.4–2.9; `radpo_kl` stayed tamed (~12–60) throughout — the kl_inv weighting held up over 3 epochs.

Evaluated ep1 (`checkpoint-902`) and ep3 (`checkpoint-2706`) on the full 6-benchmark suite. **ep2 NOT evaluated** — stopped the queue after ep1 came back ≈ ep3 (no point).

| Benchmark | ep1 (ckpt-902) | ep3 (ckpt-2706) |
|---|---|---|
| MMLU | 33.68% | 34.18% |
| TruthfulQA-MC2 | 48.29% | 49.13% |
| Winogrande | 62.04% | 61.33% |
| HellaSwag (acc_norm) | 67.79% | 68.14% |
| GSM8K (strict) | 6.14% | 4.02% |
| ARC-C (acc_norm) | 40.19% | 41.13% |
| **Average** | **43.02%** | **42.99%** |

**Read:** more riskKD epochs do nothing on this suite — ep1 ≈ ep3 (within noise; ep1's only real edge is GSM8K, where ep3 drifted back toward base-1B level). Also ep1 (43.02) ≈ the standalone 1-epoch token-wt run (43.15), so the 3-epoch run added nothing over 1 epoch either. Conclusion: this benchmark suite is the wrong instrument for ARR's gains (it measures knowledge/reasoning, which a 1B can't acquire from a preference-ratio loss) — see `Ev` in the ideas table. **Kept ep1** as the riskKD student: pushed to `vukien2301/llama-3.2-1b-riskkd-tokenwt-epoch1` (private). Per-epoch JSONs pulled to `results/eval/riskKD_tokenwt_3ep_ep1/` and `_ep3/`; `SUMMARY.md` updated.
...
| 6a | ~~3-epoch token-weighted run~~ — **DONE**. ep1 = 43.02, ep3 = 42.99 (ep2 not evaluated). Flat — more epochs don't help this suite. ep1 kept + pushed to HF. | Exp | ✅ done | See "DONE — 3-epoch token-weighted riskKD" above. Conclusion: stop iterating on epoch count; the eval is the bottleneck, not training length → see `Ev`. |
...
**Recommended order (updated):** 6a done → next: **4t (better teacher)** is the most likely thing to move the benchmark table, then **Ev (win-rate eval)** to actually see ARR's gains, then **Bx (baselines)** for a complete table, then **6b (α sweep)** / **5 (annealed-μ run, code already in)** as ablations. #1 + #2 (methods reframing) is pure writing — fold into the paper.

## Repo state

- Branch `experiments/training-runs`. `main` is at upstream `5362953`.
- Local `results/eval/` has `SUMMARY.md` + per-task JSONs for: `sft_epoch1/`, `dpo_from_epoch1/`, `riskKD/` (1ep no-wt), `riskKD_tokenwt/` (1ep token-wt), `riskKD_tokenwt_3ep_ep1/`, `riskKD_tokenwt_3ep_ep3/`.
- `proposals/loss-weighting.md` — full menu of 6 weighting strategies. `proposals/principled-weights-and-trust-region.md` — P0 #1+#2+#5 writeup (per-token weights as IS correction; per-token trust region; annealed μ).
- `scripts/run_distill_dpo.py` — has `radpo_token_weight_mode`/`radpo_token_weight_alpha` (kl_inv weighting) + `radpo_mu_anneal_start/end/frac` (annealed CVaR μ; off by default).
- `eval_script/` — lm-eval wrappers (mmlu, arc, gsm8k, hellaswag, wino, qa, all.sh — TP=8, gpu_memory_utilization=0.8, `set -o pipefail`).
- `alignment/configs.py` has the `attn_implementation` field added (missing-field regression in upstream that broke `run_sft.py`).
- HF (private, `vukien2301/*`): `llama-3.1-8b-deita-sft-teacher`, `...-epoch1`, `llama-3.1-8b-dpomix-dpo-teacher`, `llama-3.1-8b-ultrafeedback-dpo-teacher`, `llama-3.1-8b-ultrafeedback-dpo-from-epoch1`, `llama-3.2-1b-deita-sft-student`, **`llama-3.2-1b-riskkd-tokenwt-epoch1`**.
